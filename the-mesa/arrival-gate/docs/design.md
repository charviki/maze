...
stateDiagram-v2
    [*] --> Landing
    Landing --> ArrivalGate: 登录成功
    ArrivalGate --> Landing: 登出
...
6 个子系统：Director Console（在线）、The Forge、Saloon、Loop Monitor、Reveries、Abernathy Ranch（锁定）。锁定卡片底部显示 "THE MAZE IS NOT MEANT FOR YOU"。

## 构建集成

| Dockerfile | 说明 |
|---|---|
| `the-mesa/arrival-gate/Dockerfile.web` | Arrival Gate 独立构建（仅 Arrival Gate + nginx） |
| `the-mesa/Dockerfile.web` | 组合构建（Arrival Gate + Director Console + nginx） |

nginx 路由（组合构建）：
- `/` → 302 重定向到 `/arrival-gate/`
- `/arrival-gate/` → Arrival Gate 首页（alias → nginx/html/arrival-gate/）
- `/director-console/` → Director Console SPA
- `/api/` → 反向代理到 director-core:8080
