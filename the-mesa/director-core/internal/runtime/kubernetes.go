...
func (k *KubernetesRuntime) createDeployment(ctx context.Context, ns, appName string, spec *protocol.HostDeploySpec, image string) error {
	directorCoreAddr := k.kube.DirectorCoreAddr
	if directorCoreAddr == "" {
		directorCoreAddr = "director-core:8080"
	}
	// 确保 DirectorCoreAddr 带有 http:// 前缀
	if !strings.HasPrefix(directorCoreAddr, "http") {
		directorCoreAddr = "http://" + directorCoreAddr
	}
	...
	envs := []corev1.EnvVar{
		...
		{Name: "AGENT_CONTROLLER_ADDR", Value: directorCoreAddr},
		{Name: "AGENT_CONTROLLER_GRPC_ADDR", Value: deriveDirectorCoreGRPCAddr(directorCoreAddr)},
...
func (k *KubernetesRuntime) RemoveHost(ctx context.Context, name string) error {
	...
	if k.kube.VolumeType == "hostpath" && k.workspace.MountDir != "" {
		// hostPath 模式下 Director Core 容器能直接看到宿主机目录；只有显式删除这里的目录，测试和下轮部署才不会吃到旧数据。
