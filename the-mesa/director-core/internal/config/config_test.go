...
`
	...
	// 环境变量显式开启
	t.Setenv("DIRECTOR_CORE_ALLOW_PRIVATE_NETWORKS", "true")
...
func TestDockerConfig_Defaults(t *testing.T) {
	...
	if cfg.Docker.DirectorCoreAddr != "http://director-core:8080" {
		t.Errorf("期望默认 DirectorCoreAddr=http://director-core:8080, 实际=%s", cfg.Docker.DirectorCoreAddr)
	}
}
...
func TestDockerConfig_EnvOverrides(t *testing.T) {
	...
	t.Setenv("DIRECTOR_CORE_DOCKER_AGENT_BASE_IMAGE", "custom-agent:latest")
	t.Setenv("DIRECTOR_CORE_DOCKER_AGENT_DATA_DIR", "/custom/agents")
	...
	if cfg.Docker.DirectorCoreAddr != "http://my-manager:9090" {
		t.Errorf("期望 DirectorCoreAddr=http://my-manager:9090, 实际=%s", cfg.Docker.DirectorCoreAddr)
	}
