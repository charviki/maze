...
type KubernetesConfig struct {
	// Namespace Director Core 创建 Agent 资源的目标 namespace
	...
	// DirectorCoreAddr Director Core 在 K8s 集群内的 Service DNS
...
type WorkspaceConfig struct {
	// BaseDir Director Core 元数据根目录（host_specs.json/nodes.json/audit.log/host_logs）
	BaseDir string `yaml:"base_dir"`
	// MountDir Director Core 容器内对应的元数据挂载路径（用于文件操作）
...
type DockerConfig struct {
	...
	// Network 默认 Docker 网络名（Host 容器加入此网络以访问 Director Core）
...
func validate(cfg *Config) {
	...
	if cfg.Docker.AgentDataDir == "" {
		// Docker 默认沿用统一目录模型：workspace.base_dir 负责 Director Core 元数据，agents/ 子目录负责 Agent 工作目录。
	...
	if cfg.Docker.DirectorCoreAddr == "" {
		cfg.Docker.DirectorCoreAddr = "http://director-core:8080"
	}
