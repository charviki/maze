...
func (p *Proxy) getNodeAddr(ctx context.Context, nodeName string) (string, error) {
	...
	if err != nil {
		// 这里要显式映射成 gRPC 状态码；否则网关会把普通 error 统一翻译成 500。
		return "", status.Errorf(codes.Internal, "get node %q: %v", nodeName, err)
	}
	if node == nil {
		return "", status.Errorf(codes.NotFound, "node %q not found", nodeName)
	}
	if node.GrpcAddress == "" {
		return "", status.Errorf(codes.FailedPrecondition, "node %q has no gRPC address", nodeName)
	}
