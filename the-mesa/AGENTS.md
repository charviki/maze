...
- `make build-web` — The Mesa 组合前端 Docker 构建（`Dockerfile.web` 使用共享 `deps-installer` 阶段，三个 builder 复用同一 pnpm install 层）
- `make up PLATFORM=kubernetes ENV=dev|test|prod` — K8s 部署（用已有镜像；首次或镜像缺失先 `make build`）
- `make up-build PLATFORM=kubernetes ENV=dev|test|prod` — K8s 构建并部署（dev/test/prod 三套环境，= 旧 `make up` 行为）
