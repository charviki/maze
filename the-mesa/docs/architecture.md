...
The Mesa 是当前控制面域，由 **Director Core**（Go 后端，`net/http` + grpc-gateway + gRPC Server 三层架构）、**Director Console**（React 控制台前端）与 **Arrival Gate**（统一入口前端）组成。Director Core 作为代理网关和 Host 编排引擎，统一管理所有 Agent 节点，代理前端到 Agent 的所有 HTTP 和 WebSocket 请求，并通过声明式 HostSpec 持久化 + Reconciler 实现 Host 的全生命周期自动化管理。
...
## Director Core 角色：代理网关

Director Core 采用**代理网关模式**（API Gateway Pattern），核心职责：

1. **节点注册中心** — Agent 启动时向 Director Core 注册，上报地址、能力声明和状态
2. **请求代理** — 前端所有 API 请求（Session、Template、LocalConfig）经 Director Core 代理到目标 Agent
3. **WebSocket 终端代理** — 前端终端连接经 Director Core 双向代理到 Agent
...
7. **前端不直连 Agent** — 所有通信经过 Director Core，保证可观测性

## 数据流

### 节点注册流程（含令牌校验）

Director Core 为每个动态创建的 Host 预存专属令牌（`hostTokens` map），Agent 注册时校验令牌身份：
...
离线阈值定义在 [node_registry.go](../director-core/internal/repository/file/node_registry.go) 中的 `nodeOfflineThreshold = 30 * time.Second`。

### 代理请求流程（HTTP → gRPC 转发）

```
前端 → Director Core REST API（grpc-gateway 路由）
     → grpc-gateway ServeMux 匹配 proto 注解路由
     → 进程内 gRPC 调用 Director Core Server（SessionService/TemplateService/ConfigService）
...
```
前端 ws:// → Director Core /api/v1/nodes/:name/sessions/:id/ws
...
WebSocket Origin 校验使用配置化的 `allowedOrigins` 列表，为空时允许所有来源（开发模式）。详见 [ws_proxy.go](../director-core/internal/transport/ws_proxy.go)。

## 节点注册表（NodeRegistry）

### 数据结构

Node 业务对象定义在 [host_node.go](../director-core/internal/service/host_node.go)，包含：`Name`, `Address`, `ExternalAddr`, `AuthToken`, `Status`(online/offline), `RegisteredAt`, `LastHeartbeat`, `Capabilities`, `AgentStatus`, `Metadata`。
...
- **启动恢复** — Director Core 重启后从 `nodes.json` 加载已有节点，Agent 下次心跳时更新状态
...
HostSpec 的持久化实现位于 [host_spec_repository.go](../director-core/internal/repository/file/host_spec_repository.go)，原始数据结构仍使用 `protocol.HostSpec`，包含：`Name`, `Tools`, `Resources`, `AuthToken`, `Status`(pending/deploying/online/offline/failed), `ErrorMsg`, `RetryCount`, `CreatedAt`, `UpdatedAt`。
...
- **启动恢复** — Director Core 重启后从 `host_specs.json` 加载已有 HostSpec
- **业务投影** — `HostInfo` 合并逻辑已收回业务侧，由 [host_node.go](../director-core/internal/service/host_node.go) 的 `BuildHostInfo()` 负责
...
Reconciler 在 [reconciler.go](../director-core/internal/reconciler/reconciler.go) 中实现，负责确保实际运行状态趋近期望状态。

### 启动恢复（RecoverOnStartup）

Director Core 启动时执行一次：
...
- K8s 的 `rollout restart` 不会触发 Director Core 干预（Deployment 对象存在即视为健康）
...
- **启动恢复** — Director Core 重启后从 `audit.log` 加载历史日志
...
```
浏览器 → Nginx (:10800)
            ├── /                → Arrival Gate 首页 (Landing 含登录 → 主界面)
            ├── /director-console/ → Director Console SPA (Host 管理)
            ├── /api/*           → proxy_pass → director-core:8080
            └── /health          → proxy_pass → director-core:8080
...
Agent 实例（动态创建）
  └── 通过 Director Console 创建，由 Docker/K8s 运行时部署
```

### Docker Compose 编排（来自 docker-compose.yml）

- **web** — Nginx 容器，构建前端资产并托管静态文件 + 反向代理 API
- **director-core** — Go 后端容器，监听 8080 端口
  - 挂载 `/var/run/docker.sock` 以动态创建/销毁 Host 容器
  - 挂载 `~/.maze-prod/docker:/data` 以持久化 Director Core 元数据，并通过 `/data/agents` 管理 Host 工作目录
  - 安装 `docker-ce-cli` 用于执行 docker build/run/stop/rm
- **Agent 实例** — 全部通过 Director Console 动态创建，不再在 docker-compose.yml 中静态定义

### Nginx 配置要点（来自 nginx.conf）

- Arrival Gate 首页：`/` → 302 重定向到 `/arrival-gate/`，`/arrival-gate/` → alias + `try_files`
- Director Console：`/director-console/` → `alias` + `try_files $uri $uri/ /director-console/index.html`
...
1. 环境变量（`DIRECTOR_CORE_SERVER_LISTEN_ADDR` 等）
...
| workspace.mount_dir           | DIRECTOR_CORE_WORKSPACE_MOUNT_DIR     | 同 base_dir                   | Director Core 容器内的元数据挂载路径（用于文件操作）                                                             |
| docker.socket_path            | DIRECTOR_CORE_DOCKER_SOCKET_PATH      | /var/run/docker.sock          | Docker socket 路径                                                                                         |
| docker.network                | DIRECTOR_CORE_DOCKER_NETWORK          | ""                            | Docker 网络名（Host 容器加入此网络）                                                                       |
| docker.agent_base_image       | DIRECTOR_CORE_DOCKER_AGENT_BASE_IMAGE | ""                            | Agent 基础镜像名（含 agent 二进制和 entrypoint）                                                           |
| docker.agent_data_dir         | DIRECTOR_CORE_DOCKER_AGENT_DATA_DIR   | `<workspace.base_dir>/agents` | Docker Agent 宿主机根目录；容器化部署中常显式指向宿主机的 `.../docker/agents`                              |
| docker.director_core_addr     | DIRECTOR_CORE_DOCKER_DIRECTOR_CORE_ADDR | http://director-core:8080   | Director Core 在容器网络中的地址                                                                               |
