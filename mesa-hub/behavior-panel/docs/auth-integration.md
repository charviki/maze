...
这份文档说明其他模块或调用方如何基于当前最小权限系统接入 `Behavior Panel`。

当前阶段的接入前提：

- 已启用 PostgreSQL
- `authz.enabled=true`
- Manager 已成功 bootstrap `user:admin`
...
### 1. 使用全局 Bearer token 访问 Manager
...
- `agent-manager` 通过 `AGENT_MANAGER_DATABASE_*` 和 `AGENT_MANAGER_AUTHZ_*` 环境变量接入
- `authz.enabled=true` 时，Manager 启动会先跑 migration，再 bootstrap `user:admin`

### Kubernetes

- 当前仓库优先提供“连接外部 PostgreSQL”的接线方式
- `manager-config` 提供 `AGENT_MANAGER_DATABASE_HOST/PORT/NAME` 和 `AGENT_MANAGER_AUTHZ_*`
- `manager-secret` 提供 `DATABASE_USER`、`DATABASE_PASSWORD` 与 `AUTH_TOKEN`
...
1. 先复用 Manager 的 Bearer token 认证
