...
export function NodeList({ onSelectNode, selectedNodeName, onNodesChange, refreshTrigger, onViewLog }: NodeListProps) {
  const [deleteTarget, setDeleteTarget] = useState<string | null>(null);

  const { data, isLoading, refresh: refreshNodes } = usePollingWithBackoff<Host[]>({
    fetchFn: async () => {
  ...
  return (
    <div className="space-y-2">
      ...
      {!isLoading && hosts.map((host) => {
        ...
        return (
          <GlitchEffect key={host.name} isActive={host.status === 'offline' || host.status === 'failed'} className="block">
            <div
            ...
            >
              ...
              <div className="flex items-center justify-between pl-1">
                <div className="flex items-center gap-3 font-mono text-sm tracking-wide text-primary">
                  <div className="relative flex h-3 w-1.5 shrink-0">
                    {host.status === 'online' && <span className={`animate-pulse absolute inline-flex h-full w-full ${cfg.dotClass} opacity-75 ${cfg.glowClass}`}></span>}
                    {(host.status === 'pending' || host.status === 'deploying') && <span className={`animate-pulse absolute inline-flex h-full w-full ${cfg.dotClass} opacity-60`}></span>}
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  {onViewLog && (
                    <Button variant="ghost" size="icon" className="h-6 w-6 rounded-none text-muted-foreground hover:text-primary" onClick={(e) => { e.stopPropagation(); onViewLog(host.name); }}>
                      <FileText className="w-3 h-3" />
                    </Button>
                  )}
                  <Button variant="ghost" size="icon" className="h-6 w-6 rounded-none text-muted-foreground hover:text-destructive" onClick={(e) => { e.stopPropagation(); setDeleteTarget(host.name); }}>
              ...
              <div className="text-[10px] text-muted-foreground space-y-1.5 font-mono uppercase tracking-widest pl-4">
                ...
                <div className="flex justify-between items-center opacity-60">
                  {host.address ? (
                    <>
                      ...
                      <span>SYS_BEAT: {host.last_heartbeat ? formatTimeAgo(host.last_heartbeat) : 'N/A'}</span>
                    </>
                  ) : (
                    <>
                ...
                {host.error_msg && (
                  <div className="text-red-400/80 text-[9px] truncate">
                    ERR: {host.error_msg}
                  </div>
                )}
              </div>
