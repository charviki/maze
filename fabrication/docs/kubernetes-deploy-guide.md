...
- `dev` / `test` / `production` overlay 当前都自带 `postgresql` Deployment + Service，确保 Manager 的数据库配置与实际工作负载一致。
...
└── overlays/
    ...
    └── production/                       # 生产环境 overlay
        ...
        ├── manager-configmap.yaml        # PVC + Agent 运行时配置
        └── manager-deployment.yaml       # PVC 挂载 + Docker socket 挂载
```
...
| Manager 数据卷  | hostPath (`/tmp/maze-dev/kubernetes/`)              | hostPath (`/tmp/maze-test/kubernetes/`)              | PVC (`manager-data`) |
...
- `agent-manager` Pod 必须挂载 `/var/run/docker.sock`
- `maze-manager` 镜像必须包含 `docker` CLI
