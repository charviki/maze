# Kubernetes 部署指南

Maze 支持在 Kubernetes 上运行，通过 `fabrication/kubernetes/` 中的 Makefile 和 Kustomize overlay 管理部署。内置三种环境配置：

- **dev**（默认）— 开发环境，hostPath 挂到 `HOST_DATA_DIR`（默认 `~/.maze-dev/kubernetes`，通过 envsubst 注入），内置 PostgreSQL，动态构建 Agent 镜像
- **test** — 集成测试环境，hostPath 挂到 `HOST_DATA_DIR`（默认 `~/.maze-test/kubernetes`，通过 envsubst 注入），内置 PostgreSQL，`make test-integration` 自动管理
- **production** — 生产环境，PVC 持久化，内置 PostgreSQL，动态构建 Agent 镜像

## 前置条件

- Docker Desktop 已启用 Kubernetes（Settings → Kubernetes → Enable Kubernetes）
- `kubectl` 可正常连接集群（`kubectl cluster-info` 返回正常）
- 项目代码在本地已就绪

## 快速开始

```bash
# 一键部署（构建镜像 + 部署 K8s 资源 + 等待就绪）
make up

# 启动本地代理
make proxy

# 浏览器打开 http://localhost:7080

# 清理
make down
```

## 环境变量体系

Makefile 使用两个正交变量控制部署行为：

- **`PLATFORM`** = `docker` | `kubernetes`（默认 `kubernetes`）— 运行时类型
- **`ENV`** = `dev` | `test` | `prod`（默认 `dev`）— 环境级别

```bash
make up                              # kubernetes × dev  → maze-dev namespace
make up PLATFORM=docker              # docker × dev      → Docker Compose maze-dev
make up ENV=prod                     # kubernetes × prod → maze-prod namespace
make test-integration                # 固定 ENV=test
make test-integration PLATFORM=docker # docker × test
```

### ENV 派生属性

| 属性          | ENV=dev       | ENV=test       | ENV=prod       |
| ------------- | ------------- | -------------- | -------------- |
| K8s Namespace | `maze-dev`    | `maze-test`    | `maze-prod`    |
| Docker 宿主机目录 | `~/.maze-dev` | `~/.maze-test` | `~/.maze-prod` |
| K8s 宿主机目录 | `~/.maze-dev/kubernetes` | `~/.maze-test/kubernetes` | `PVC` |
| PORT_DIRECTOR_CORE  | `7090`        | `9090`         | `8090`         |
| PORT_THE_FORGE  | `7091`        | `9091`         | `8091`         |
| PORT_WEB      | `7080`        | `9080`         | `10800`        |

## Make 命令一览

```bash
make help               # 显示所有命令
```

| 命令                  | 说明                                                           |
| --------------------- | -------------------------------------------------------------- |
| `make up`             | 构建全部镜像 + 部署 K8s 资源 + 等待就绪                        |
| `make down`           | 停止服务并保留 PostgreSQL 数据                                  |
| `make status`         | 查看 Pod / Service / PVC 状态                                  |
| `make proxy`          | 启动 port-forward 代理（Web + Director Core + The Forge），Ctrl+C 停止 |
| `make proxy-web`      | 只代理 Web 前端                                                |
| `make proxy-director-core`  | 只代理 Director Core API                                       |
| `make build`          | 构建全部镜像（Director Core + Web + Agent）                    |
| `make build-director-core`  | 只构建 Director Core 镜像                                      |
| `make build-web`      | 只构建 Web 镜像                                                |
| `make build-agent`    | 只构建 Agent 基础镜像                                          |
| `make deploy`         | 只部署 K8s 资源（不构建镜像）                                  |
| `make update-director-core` | 重建 Director Core 镜像 + 滚动重启                             |
| `make update-web`     | 重建 Web 镜像 + 滚动重启                                       |
| `make update-agent`   | 重建 Agent 基础镜像（新创建的 Host 使用新镜像）                |
| `make update-all`     | 全部更新                                                       |
| `make destroy`        | 停止服务并删除 PostgreSQL 数据（破坏性操作）                    |
| `make docker-clean`   | 清理超过 7 天的未使用镜像、构建缓存和匿名卷                    |

部署等待策略说明：
- `make up` / `make deploy` 只等待当前 overlay 实际声明的 deployment，避免对未纳入该环境的工作负载做错误的 rollout 检查。
- `dev` / `test` / `production` overlay 当前都自带 `postgresql` Deployment + Service，确保 Director Core 的数据库配置与实际工作负载一致。
- `production` 使用独立 `postgresql-data` PVC 持久化数据库数据；如需切换到外部数据库，可移除 `postgres.yaml` 并保留相同凭据键。

### 环境选择

通过 `ENV` 参数切换 overlay：

```bash
make up                          # 默认 ENV=dev
make up ENV=prod                 # 生产环境
make deploy ENV=prod             # 只部署，不构建镜像
```

## 更新流程

代码修改后，使用 `update-*` 命令重建镜像并滚动重启：

```bash
# 只更新 Director Core
make update-director-core

# 只更新 Web 前端
make update-web

# 更新 Agent 基础镜像（已有 Agent Pod 不会自动更新，需通过 Director Core API 删除重建）
make update-agent

# 全部更新
make update-all
```

## 本地访问

K8s Service 默认 ClusterIP（集群内网），需要通过 port-forward 代理到本地。dev 环境的 Web 服务使用 LoadBalancer 暴露，可通过 `maze.local` 域名直接访问（需在 `/etc/hosts` 添加 `127.0.0.1 maze.local`）：

```bash
# 同时代理 Web 和 Director Core（最常用）
make proxy
# Web:      http://localhost:7080
# Director Core:  http://localhost:7090/health
# The Forge:  http://localhost:7091

# 只代理某一个
make proxy-web        # 只代理前端
make proxy-director-core    # 只代理 Director Core API

# 自定义端口
make proxy PORT_WEB=3000 PORT_DIRECTOR_CORE=9000
```

## Agent 动态构建

K8s 模式下 **复用 Docker 模式的动态构建逻辑**，不需要提前预构建所有工具组合的镜像：

1. 用户通过前端或 API 选择工具组合（如 claude + go + python）
2. Manager 调用 `builder.GenerateHostDockerfile()` 生成 Dockerfile
3. Manager 通过 Docker socket 执行 `docker build` 构建镜像
4. Manager 通过 K8s API 创建 Deployment 引用该镜像

这要求 Manager Pod 挂载宿主机的 Docker socket。构建出的镜像直接出现在当前 Docker daemon 的本地镜像仓库，K8s 通过 `imagePullPolicy` 访问。镜像按工具集组合标签（combo tag）缓存，同一工具集的多个 Host 共享同一镜像（如 `maze-agent:claude-go`），Host 删除时镜像保留。

```
用户选工具 → Manager 生成 Dockerfile → docker build → K8s Deployment
                                        ↑
                              Docker socket (hostPath 挂载)
```

## PostgreSQL 运维

### WAL 自动修复

dev/test overlay 的 PostgreSQL 使用 hostPath 存储数据。Docker Desktop 的 VirtioFS 文件系统在 VM 挂起/重启时可能导致 WAL checkpoint 截断，使数据库无法启动。

dev overlay 的 `postgres.yaml` 包含自动修复 entrypoint：启动前检测 WAL 完整性，损坏时先尝试 `pg_ctl` 恢复，失败后用 `pg_resetwal` 重置 WAL。同时配置了 `sync` + `pg_ctl stop` 的 preStop hook 和 120s 优雅终止期。

## 存储路径

Maze 的持久化目录按运行时区分：

```
~/.maze-dev/                          # 开发环境 (ENV=dev)
├── docker/                           # Docker Compose 模式
│   ├── nodes.json                    # Manager 持久化
│   ├── host_specs.json               # Manager 持久化
│   ├── audit.log                     # Manager 持久化
│   ├── host_logs/                    # Manager 运行日志
│   └── agents/                       # Agent 工作目录
├── kubernetes/                       # K8s hostPath（envsubst 注入）
│   ├── nodes.json                    # Manager 持久化
│   ├── host_specs.json               # Manager 持久化
│   ├── audit.log                     # Manager 持久化
│   ├── host_logs/                    # Manager 运行日志
│   └── agents/                       # Agent 工作目录
├── postgresql/                       # K8s PostgreSQL 数据（envsubst 注入）
│   └── data/

~/.maze-test/                         # 集成测试 (ENV=test)
├── docker/                           # Docker 集成测试
│   ├── nodes.json                    # Manager 持久化
│   ├── host_specs.json               # Manager 持久化
│   ├── audit.log                     # Manager 持久化
│   ├── host_logs/                    # Manager 运行日志
│   └── agents/                       # Agent 工作目录
├── kubernetes/                       # K8s hostPath（envsubst 注入）
│   ├── nodes.json                    # Manager 持久化
│   ├── host_specs.json               # Manager 持久化
│   ├── audit.log                     # Manager 持久化
│   ├── host_logs/                    # Manager 运行日志
│   └── agents/                       # Agent 工作目录
├── postgresql/                       # K8s PostgreSQL 数据（envsubst 注入）
│   └── data/

~/.maze-prod/                         # 生产环境 Docker 数据 (ENV=prod)
├── docker/                           # Docker 生产
│   ├── nodes.json                    # Manager 持久化
│   ├── host_specs.json               # Manager 持久化
│   ├── audit.log                     # Manager 持久化
│   ├── host_logs/                    # Manager 运行日志
│   └── agents/                       # Agent 工作目录
# Kubernetes production 使用 PVC，不依赖宿主机固定目录
```

| ENV  | Manager 数据                        | Agent 数据                                       | 访问方式                       |
| ---- | ----------------------------------- | ------------------------------------------------ | ------------------------------ |
| dev  | hostPath `~/.maze-dev/kubernetes/`  | hostPath `~/.maze-dev/kubernetes/agents/{name}`  | 宿主机直接可见                 |
| test | hostPath `~/.maze-test/kubernetes/` | hostPath `~/.maze-test/kubernetes/agents/{name}` | 集成测试自动管理               |
| prod | PVC `director-core-data`                  | PVC（动态创建）                                  | `kubectl exec` 或 `kubectl cp` |

## Kustomize Overlay 结构

```
fabrication/kubernetes/
├── Makefile                              # 部署管理入口（支持 PLATFORM × ENV 参数）
├── base/                                 # 公共资源（三种环境共享）
│   ├── kustomization.yaml
│   ├── namespace.yaml                    # maze-prod namespace
│   ├── director-core-rbac.yaml                 # ServiceAccount + Role + RoleBinding（含 pods/log 权限）
│   ├── director-core-secret.yaml               # Auth Token Secret（需填入实际值）
│   ├── director-core-pvc.yaml                  # Manager PVC（仅 production 使用）
│   ├── director-core-service.yaml              # Manager ClusterIP Service :8080(HTTP) + :9090(gRPC)
│   ├── the-forge-deployment.yaml          # The Forge Deployment
│   ├── the-forge-secret.yaml              # The Forge 数据库凭据
│   ├── the-forge-service.yaml             # The Forge ClusterIP Service :8080(HTTP) + :9090(gRPC)
│   ├── web-configmap.yaml                # Nginx 配置模板（envsubst 变量注入）
│   ├── web-deployment.yaml               # Web Deployment
│   ├── web-service.yaml                  # Web ClusterIP Service :80
│   └── web-ingress.yaml                  # Ingress（需 Ingress Controller）
└── overlays/
    ├── dev/                              # 开发环境 overlay
    │   ├── kustomization.yaml            # namespace: maze-dev
    │   ├── postgres.yaml                 # PostgreSQL Deployment + Service + WAL 自动修复 entrypoint
    │   ├── director-core-configmap.yaml  # hostPath + 动态构建 + Docker socket
    │   └── director-core-deployment.yaml # hostPath 挂载 + Docker socket 挂载
    ├── test/                             # 集成测试 overlay
    │   ├── kustomization.yaml            # namespace: maze-test，删除 web/namespace 资源
    │   ├── postgres.yaml                 # 测试专用 PostgreSQL Deployment + Service
    │   ├── director-core-configmap.yaml  # hostPath + 动态构建 + Docker socket
    │   └── director-core-deployment.yaml # hostPath 挂载 + Docker socket + 声明 8080/9090 端口
    └── production/                       # 生产环境 overlay
        ├── kustomization.yaml
        ├── postgres.yaml                 # 生产 PostgreSQL Deployment + Service + PVC
        ├── director-core-configmap.yaml  # PVC + Agent 运行时配置
        └── director-core-deployment.yaml # PVC 挂载 + Docker socket 挂载
```

### 三套 Overlay 差异

| 配置项          | dev                                               | test                                               | production           |
| --------------- | ------------------------------------------------- | -------------------------------------------------- | -------------------- |
| Namespace       | `maze-dev`                                        | `maze-test`                                        | `maze-prod`          |
| Manager 数据卷  | hostPath (`~/.maze-dev/kubernetes/`)              | hostPath (`~/.maze-test/kubernetes/`)              | PVC (`director-core-data`) |
| Agent 数据卷    | hostPath (`~/.maze-dev/kubernetes/agents/{name}`) | hostPath (`~/.maze-test/kubernetes/agents/{name}`) | PVC（动态创建）      |
| Agent 镜像      | 动态构建（docker build via Docker socket）        | 动态构建                                           | 动态构建（需 Docker socket） |
| VolumeType      | `hostpath`                                        | `hostpath`                                         | `pvc`                |
| ImagePullPolicy | `Never`                                           | `Never`                                            | 默认 `Never`，可覆盖 |
| Docker socket   | ✅ 挂载                                           | ✅ 挂载                                            | ✅ 挂载              |
| PostgreSQL      | 内置 Deployment + Service                         | 内置 Deployment + Service                          | 内置 Deployment + PVC + Service |
| Web 前端        | ✅                                                | ❌（测试不需要）                                   | ✅                   |
| 目录自动创建    | ✅                                                | ✅                                                 | ❌                   |

## 部署前必做

### 1. 填入 JWT Secret 与管理员密码

编辑 `base/director-core-secret.yaml`：

```yaml
stringData:
  DIRECTOR_CORE_JWT_SECRET: 'your-jwt-secret-here'
  DIRECTOR_CORE_DEFAULT_ADMIN_PASSWORD: 'your-admin-password-here'
```

### 2. 镜像来源与运行时参数

当前三套 overlay 都直接在清单中声明镜像、`imagePullPolicy` 和 Agent 运行时参数，不再通过 `Makefile` 生成临时 `overlay-runtime.env`。
这意味着 Kubernetes 部署完全由仓库内的静态 Kustomize 清单决定，`make up` 只负责构建镜像并执行部署。

默认情况下，`make up` 复用本地刚构建的镜像：

```bash
make up ENV=prod
```

如需切到远程仓库，直接修改对应 overlay 或 base 中的镜像字段后再部署：

```bash
# 例如修改 production overlay / base 中的 image 与 imagePullPolicy
make up ENV=prod
```

## 容器命名规范

| 组件              | Docker Compose (service name) | K8s (Deployment/Service) |
| ----------------- | ----------------------------- | ------------------------ |
| Manager           | `director-core`               | `director-core`          |
| Web 前端          | `web`                         | `web`                    |
| Agent（动态创建） | `agent-claude-1` 等           | `maze-agent-{name}`      |

## 镜像策略

### 开发/测试环境

Manager 挂载 Docker socket，Agent 镜像按需动态构建，无需提前预构建：

| 镜像                  | 说明                                                         |
| --------------------- | ------------------------------------------------------------ |
| `maze-director-core:latest` | Manager 后端（`make build-director-core`）                         |
| `maze-the-forge:latest`     | The Forge 知识库服务（`make build-the-forge`）               |
| `maze-web:latest`     | Web Nginx 前端（`make build-web`）                           |
| `maze-agent:latest`   | Agent 基础镜像（`make build-agent`，动态构建的 FROM 基础层） |
| `maze-agent:{combo}`  | 按工具集组合动态构建（如 `maze-agent:claude-go`）            |

### 生产环境

默认情况下，`production` overlay 复用本地 `make build` 产出的 Manager / Web / Agent 基础镜像；
创建 Host 时，Manager 仍会像开发环境一样通过 Docker socket 动态构建工具组合镜像。

| 镜像                  | 默认值                |
| --------------------- | --------------------- |
| Manager               | `maze-director-core:latest` |
| Web                   | `maze-web:latest`     |
| Agent base image      | `maze-agent:latest`   |
| Agent image prefix    | `maze-agent`          |
| Agent image tag       | `latest`              |
| ImagePullPolicy       | `Never`               |

要求：

- `director-core` Pod 必须挂载 `/var/run/docker.sock`
- `maze-director-core` 镜像必须包含 `docker` CLI
- 当前集群节点需要能直接使用这套 Docker daemon 构建出的镜像

如需对接远程镜像仓库，直接调整 overlay 中的 `image` / `imagePullPolicy` 即可；Host 运行时镜像仍依赖动态构建。

## RBAC 权限

Manager 需要以下 K8s API 权限来动态管理 Agent：

| 资源                     | 操作           | 用途                                   |
| ------------------------ | -------------- | -------------------------------------- |
| `apps/deployments`       | CRUD           | 创建/删除 Agent Deployment             |
| `pods`                   | get/list/watch | 查询 Agent Pod 状态                    |
| `pods/log`               | get            | 获取 Agent 运行时日志                  |
| `services`               | CRUD           | 创建/删除 Agent Service                |
| `persistentvolumeclaims` | CRUD           | 创建/删除 Agent PVC（production 模式） |

## Docker Compose vs Kubernetes 差异

| 维度       | Docker Compose                            | Kubernetes (dev)                            | Kubernetes (production)                 |
| ---------- | ----------------------------------------- | ------------------------------------------- | --------------------------------------- |
| 部署方式   | `docker compose up`                       | `make up`                                   | `make up ENV=prod`                      |
| Agent 构建 | Docker socket 动态构建                    | Docker socket 动态构建（相同）              | Docker socket 动态构建                  |
| Agent 启动 | `docker run`                              | K8s API 创建 Deployment + Service           | K8s API 创建 Deployment + PVC + Service |
| 持久化     | bind mount (`~/.maze-dev/docker/agents/`) | hostPath (`~/.maze-dev/kubernetes/agents/`) | PVC                                     |
| 网络发现   | Docker DNS                                | K8s Service DNS                             | K8s Service DNS                         |
| 配置注入   | docker-compose.yml                        | ConfigMap + Secret                          | ConfigMap + Secret                      |
| 更新组件   | `docker compose up -d --build`            | `make update-*`                             | `make update-*`，同时要求 Manager 可访问 Docker daemon |

## 集成测试

集成测试固定使用 `ENV=test`，支持 Docker 和 Kubernetes 两种平台：

```bash
# Docker 集成测试
make test-integration PLATFORM=docker

# Kubernetes 集成测试（自动部署 test 环境 + port-forward + 测试 + 清理）
make test-integration PLATFORM=kubernetes

# 运行单个测试
make test-integration PLATFORM=docker TEST_NAME=TestHostCreateOnline
```

Kubernetes 集成测试流程：

1. 自动创建 `maze-test` namespace
2. 用 test overlay 部署 Manager
3. 等待 Pod 就绪
4. 启动 port-forward（9090:8080 + 9091:8081）
5. 运行测试
6. 无论成功/失败/中断，自动清理 port-forward 和 namespace
