...
| `make proxy`          | 启动 port-forward 代理（Web + Director Core + Gateway），Ctrl+C 停止 |
...
这要求 Manager Pod 挂载宿主机的 Docker socket。构建出的镜像直接出现在当前 Docker daemon 的本地镜像仓库，K8s 通过 `imagePullPolicy` 访问。
...
└── overlays/
    ...
    │   ├── postgres.yaml                 # 本地开发用 PostgreSQL Deployment + Service
