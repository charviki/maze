...
import type { SessionTemplate, EnvDef, FileDef, ConfigLayer, ConfigFileSnapshot, ConfigFileUpdate } from '../../types';
...
const emptyTemplate = (): SessionTemplate => ({
  id: '', name: '', command: '', description: '', icon: '📦', builtin: false,
...
export function TemplateManager({ open, onClose, apiClient }: TemplateManagerProps) {
  ...
  const startEdit = async (tpl: SessionTemplate) => {
    ...
    setGlobalFiles(configRes.data.files.map(file => ({
      ...file,
      ...
      if (allowRealGlobalWrite && hasConfigChanges) {
        ...
        if (configRes.status !== 'ok' || !configRes.data) {
          if (configRes.code === 'config_conflict') {
            ...
            throw new Error(conflictPaths ? `配置已变更，请重新加载后再修改：${conflictPaths}` : (configRes.message || '配置已变更，请重新加载后再修改'));
          }
  ...
  const addDefaultFile = () => {
    ...
    setEditing(prev => prev ? {
      ...prev,
      defaults: { ...prev.defaults, files: [...prev.defaults.files, { path: '', content: '' }] },
    } : null);
  };
  ...
  const addEnvDef = () => {
    ...
    setEditing(prev => prev ? {
      ...
      session_schema: {
        ...prev.session_schema,
        env_defs: [...prev.session_schema.env_defs, { key: '', label: '', required: false, placeholder: '', sensitive: false }],
      },
    } : null);
  };
  ...
  const updateEnvDef = (idx: number, field: keyof EnvDef, val: string | boolean) => {
    ...
    setEditing(prev => prev ? { ...prev, session_schema: { ...prev.session_schema, env_defs: defs } } : null);
  };
  ...
  const removeEnvDef = (idx: number) => {
    ...
    setEditing(prev => prev ? {
      ...prev,
      session_schema: { ...prev.session_schema, env_defs: prev.session_schema.env_defs.filter((_, i) => i !== idx) },
    } : null);
  };
  ...
  const addFileDef = () => {
    ...
    setEditing(prev => prev ? {
      ...
      session_schema: {
        ...prev.session_schema,
        file_defs: [...prev.session_schema.file_defs, { path: '', label: '', required: false, default_content: '' }],
      },
    } : null);
  };
  ...
  const updateFileDef = (idx: number, field: keyof FileDef, val: string | boolean) => {
    ...
    defs[idx] = { ...defs[idx], [field]: val };
    setEditing(prev => prev ? { ...prev, session_schema: { ...prev.session_schema, file_defs: defs } } : null);
  };
  ...
  const removeFileDef = (idx: number) => {
    ...
    setEditing(prev => prev ? {
      ...prev,
      session_schema: { ...prev.session_schema, file_defs: prev.session_schema.file_defs.filter((_, i) => i !== idx) },
    } : null);
  };

  const updateGlobalFileContent = (path: string, content: string) => {
    setGlobalFiles(prev => prev.map(file => file.path === path ? { ...file, content } : file));
  };

  const hasConfigChanges = useMemo(() => {
    if (isNew) return false;
    return globalFiles.some(file => file.content !== file.original_content);
  }, [globalFiles, isNew]);
            ...
            <>
              <div>
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-medium text-sm text-muted-foreground flex items-center gap-1"><Lock className="w-4 h-4" /> 内置模板</h3>
                  <Button size="sm" variant="outline" onClick={startNew}><Plus className="w-4 h-4 mr-1" /> 新建</Button>
                </div>
                {builtinTemplates.map(tpl => (
                  <div key={tpl.id} className="flex items-center justify-between py-1.5 px-2 hover:bg-muted rounded text-sm gap-2">
                    <span className="truncate min-w-0">{tpl.icon} {tpl.name} <span className="text-muted-foreground">- {tpl.command || '(empty)'}</span></span>
                    <div className="flex gap-1 shrink-0">
                      <Button size="sm" variant="ghost" title="基于此模板创建副本" onClick={() => startClone(tpl)}><Copy className="w-3 h-3" /></Button>
                      <Button size="sm" variant="ghost" title="编辑模板" onClick={() => void startEdit(tpl)}><Edit2 className="w-3 h-3" /></Button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {userTemplates.length > 0 && (
                <div>
                  ...
                  {userTemplates.map(tpl => (
                    <div key={tpl.id} className="flex items-center justify-between py-1.5 px-2 hover:bg-muted rounded text-sm gap-2">
                      <span className="truncate min-w-0">{tpl.icon} {tpl.name} <span className="text-muted-foreground">- {tpl.command}</span></span>
                      <div className="flex gap-1 shrink-0">
                        <Button size="sm" variant="ghost" title="基于此模板创建副本" onClick={() => startClone(tpl)}><Copy className="w-3 h-3" /></Button>
                        <Button size="sm" variant="ghost" title="编辑模板" onClick={() => void startEdit(tpl)}><Edit2 className="w-3 h-3" /></Button>
                        <Button size="sm" variant="ghost" className="text-red-500" onClick={() => handleDelete(tpl.id)}><Trash2 className="w-3 h-3" /></Button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {isNew && (
                    <div>
                      ...
                      <Input value={editing.id} onChange={e => updateField('id', e.target.value)} placeholder="例如 my-agent" />
                    </div>
                  )}
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">模板名称</label>
                    <Input value={editing.name} onChange={e => updateField('name', e.target.value)} placeholder="例如 Claude Code" disabled={metadataLocked} />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">启动命令</label>
                    <Input value={editing.command} onChange={e => updateField('command', e.target.value)} placeholder="例如 claude" disabled={metadataLocked} />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">图标</label>
                    <Input value={editing.icon} onChange={e => updateField('icon', e.target.value)} placeholder="🤖" className="w-20" disabled={metadataLocked} />
                  </div>
                  <div className="col-span-2">
                    <label className="text-xs text-muted-foreground mb-1 block">描述</label>
                    <Input value={editing.description} onChange={e => updateField('description', e.target.value)} placeholder="简短描述这个模板" disabled={metadataLocked} />
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isNew && <Button size="sm" variant="ghost" onClick={addDefaultFile}><Plus className="w-3 h-3" /></Button>}
                </div>
                {isNew ? editing.defaults.files.map((f, i) => (
                  <div key={i} className="border border-border rounded p-2 mb-2 space-y-1">
                    <div className="flex items-center gap-2">
                      <div className="flex-1">
                        ...
                        <Input value={f.path} onChange={e => updateDefaultFile(i, 'path', e.target.value)} className="text-sm" placeholder="~/.claude/settings.json" />
                      </div>
                      <button onClick={() => removeDefaultFile(i)} className="text-red-500 mt-4"><Trash2 className="w-3 h-3" /></button>
                    </div>
                    <div>
                      <label className="text-xs text-muted-foreground">默认内容</label>
                      <textarea
                        value={f.content}
                        onChange={e => updateDefaultFile(i, 'content', e.target.value)}
                  ...
                  <div key={f.path} className="border border-border rounded p-3 mb-2 space-y-2">
                    ...
                    <div>
                      ...
                      <textarea
                        value={f.content}
                        onChange={e => updateGlobalFileContent(f.path, e.target.value)}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Button size="sm" variant="ghost" onClick={addEnvDef}><Plus className="w-3 h-3" /></Button>
                </div>
                {editing.session_schema.env_defs.map((d, i) => (
                  <div key={i} className="border border-border rounded p-3 mb-3 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => removeEnvDef(i)} className="text-red-500"><Trash2 className="w-3 h-3" /></button>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-xs text-muted-foreground">Key</label>
                        <Input value={d.key} onChange={e => updateEnvDef(i, 'key', e.target.value)} className="text-sm" placeholder="ANTHROPIC_API_KEY" />
                      </div>
                      <div>
                        <label className="text-xs text-muted-foreground">显示名称</label>
                        <Input value={d.label} onChange={e => updateEnvDef(i, 'label', e.target.value)} className="text-sm" placeholder="API Key" />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-xs text-muted-foreground">占位提示</label>
                        <Input value={d.placeholder} onChange={e => updateEnvDef(i, 'placeholder', e.target.value)} className="text-sm" placeholder="sk-ant-..." />
                      </div>
                      <div className="flex items-end gap-3 pb-1">
                        <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
                          <input type="checkbox" checked={d.required} onChange={e => updateEnvDef(i, 'required', e.target.checked)} className="rounded" />
                          必填
                        </label>
                        <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
                          <input type="checkbox" checked={d.sensitive} onChange={e => updateEnvDef(i, 'sensitive', e.target.checked)} className="rounded" />
                          敏感
                        </label>
                      </div>
                    </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isNew && <Button size="sm" variant="ghost" onClick={addFileDef}><Plus className="w-3 h-3" /></Button>}
                </div>
                {editing.session_schema.file_defs.map((d, i) => (
                  <div key={i} className="border border-border rounded p-3 mb-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium">File #{i + 1}</span>
                      {isNew && <button onClick={() => removeFileDef(i)} className="text-red-500"><Trash2 className="w-3 h-3" /></button>}
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-xs text-muted-foreground">文件路径</label>
                        <Input value={d.path} onChange={e => updateFileDef(i, 'path', e.target.value)} className="text-sm" placeholder="CLAUDE.md" readOnly={existingTemplate} />
                      </div>
                      <div>
                        <label className="text-xs text-muted-foreground">显示名称</label>
                        <Input value={d.label} onChange={e => updateFileDef(i, 'label', e.target.value)} className="text-sm" placeholder="项目记忆文件" disabled={metadataLocked} />
                      </div>
                    </div>
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        <label className="text-xs text-muted-foreground">默认内容</label>
                        <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
                          <input type="checkbox" checked={d.required} onChange={e => updateFileDef(i, 'required', e.target.checked)} className="rounded" disabled={metadataLocked} />
                          必填
                        </label>
                      </div>
                      <textarea
                        value={d.default_content}
                        onChange={e => updateFileDef(i, 'default_content', e.target.value)}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="outline" onClick={resetEditor}>取消</Button>
...
function cloneTemplate(tpl: SessionTemplate): SessionTemplate {
  return {
    ...tpl,
    defaults: { env: { ...tpl.defaults.env }, files: tpl.defaults.files.map(file => ({ ...file })) },
    session_schema: {
