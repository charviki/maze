...
export function RadarView({ className, nodes }: RadarViewProps) {
  ...
  return (
    <div className={cn("relative aspect-square w-full h-full rounded-full border border-primary/30 bg-background/40 overflow-hidden shadow-[0_0_15px_rgba(0,255,255,0.15)]", className)}>
      <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full">
        <circle cx="50" cy="50" r="45" fill="none" stroke="hsl(var(--primary) / 0.2)" strokeWidth="0.5" />
        <circle cx="50" cy="50" r="30" fill="none" stroke="hsl(var(--primary) / 0.2)" strokeWidth="0.5" />
        <circle cx="50" cy="50" r="15" fill="none" stroke="hsl(var(--primary) / 0.2)" strokeWidth="0.5" />

        <line x1="50" y1="5" x2="50" y2="95" stroke="hsl(var(--primary) / 0.15)" strokeWidth="0.3" />
        <line x1="5" y1="50" x2="95" y2="50" stroke="hsl(var(--primary) / 0.15)" strokeWidth="0.3" />

        <g className="radar-sweep-svg">
          <line x1="50" y1="50" x2="95" y2="50" stroke="hsl(var(--primary) / 0.8)" strokeWidth="0.8" />
        ...
        {nodes && nodes.length > 0 ? (
          ...
          <>
            <circle cx="60" cy="35" r="1.5" className="radar-blip-svg" style={{ animationDelay: '0.5s' }} />
            <circle cx="40" cy="65" r="1.5" className="radar-blip-svg" style={{ animationDelay: '2.1s' }} />
            <circle cx="25" cy="45" r="1.5" className="radar-blip-svg" style={{ animationDelay: '3.2s' }} />
          </>
        )}

        <circle cx="50" cy="50" r="1.5" className="fill-primary" style={{ filter: 'drop-shadow(0 0 3px hsl(var(--primary)))' }} />
      </svg>
    </div>
  );
}
