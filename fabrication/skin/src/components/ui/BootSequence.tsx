...
export function BootSequence({ onComplete, division = 'the-mesa' }: BootSequenceProps) {
  ...
  useEffect(() => {
    if (!settings.bootSequence || prefersReducedMotion) {
    ...
    timers.push(
      setTimeout(() => {
        setPhase('brand');
      }, BOOT_PHASE_DELAYS.brand),
    );
    timers.push(
      setTimeout(() => {
        setPhase('diag');
      }, BOOT_PHASE_DELAYS.diag),
    );
    timers.push(
      setTimeout(() => {
        setPhase('cognitive');
      }, BOOT_PHASE_DELAYS.cognitive),
    );
    timers.push(
      setTimeout(() => {
        setPhase('awaken');
      }, BOOT_PHASE_DELAYS.awaken),
    );
    timers.push(
      setTimeout(() => {
        setPhase('done');
      }, BOOT_PHASE_DELAYS.done),
    );
    ...
    <div className="fixed inset-0 z-[100] bg-black text-cyan-400 font-mono overflow-hidden">
      {phase === 'wake' && (
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          {/* 水平扫描线 */}
          <div
            className="absolute h-[1px] bg-cyan-400/60 boot-wake-ray"
            style={{
              animation: 'boot-wake-ray-h 600ms 200ms ease-out forwards',
              left: 0,
              right: 0,
              marginLeft: 'auto',
              marginRight: 'auto',
          ...
          {/* 对角线扫描线（左上→右下） */}
          <div
            className="absolute h-[1px] w-0 bg-cyan-400/40 boot-wake-ray origin-left"
            style={{
              animation: 'boot-wake-ray-d 700ms 250ms ease-out forwards',
              left: '50%',
              top: '50%',
            }}
          />
          {/* 对角线扫描线（右上→左下，镜像） */}
          <div
            className="absolute h-[1px] w-0 bg-cyan-400/40 boot-wake-ray origin-right"
            style={{
              animation: 'boot-wake-ray-d 700ms 300ms ease-out forwards',
              right: '50%',
              top: '50%',
              transform: 'scaleX(-1)',
            }}
      ...
      {(phase === 'brand' || phase === 'diag') && (
        ...
        >
          <pre
            className="text-cyan-400 text-[6px] sm:text-[8px] md:text-[10px] leading-tight text-center"
            style={{ animation: 'boot-text-reveal 800ms ease-out forwards' }}
          >
            {DELOS_ASCII}
          </pre>
          <div
            className="mt-4 text-[10px] sm:text-xs tracking-[0.3em] text-cyan-300/80 uppercase"
            style={{ animation: 'boot-text-reveal 600ms 300ms ease-out both' }}
          >
            DELOS INCORPORATED // DIVISION: {divisionLabel}
          </div>
          <div
            className="mt-1 text-[8px] sm:text-[10px] tracking-widest text-cyan-400/50"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 pb-16">
          <div className="max-w-xl mx-auto">
            {visibleLines.map((line, i) => (
              <div
                key={i}
                className="text-xs sm:text-sm leading-relaxed text-cyan-400/90"
      ...
      {phase === 'cognitive' && (
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          <div className="text-center relative">
            {!showOnline ? (
              <div
                className="text-lg sm:text-2xl md:text-3xl tracking-[0.5em] text-cyan-400 uppercase font-bold"
              ...
              <div className="relative">
                ...
                {/* 第三层扩散光环（更大延迟） */}
                <div
                  className="absolute inset-0 rounded-full bg-cyan-300/10 blur-3xl"
      ...
      {phase === 'awaken' && (
        ...
        >
          {/* 意识扩散：从中心发出的多层能量波 */}
          <div
            className="absolute w-4 h-4 rounded-full bg-cyan-400/40 blur-md"
            style={{ animation: 'boot-consciousness-wave 600ms ease-out forwards' }}
          />
          <div
            className="absolute w-4 h-4 rounded-full bg-cyan-400/30 blur-lg"
            style={{ animation: 'boot-consciousness-wave 800ms 100ms ease-out forwards' }}
          />
          {/* 残留的 ONLINE 文字渐隐 */}
          <div
            className="text-lg sm:text-2xl md:text-3xl tracking-[0.5em] text-cyan-400 uppercase font-bold opacity-60"
