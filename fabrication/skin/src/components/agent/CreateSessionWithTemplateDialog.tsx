...
export function CreateSessionWithTemplateDialog({
  ...
  const relativeDir = useMemo(
    () => (relativeDirTouched ? relativeDirInput : createName.trim()),
    [relativeDirTouched, relativeDirInput, createName],
  );
  ...
  const handleRelativeDirChange = (value: string) => {
    ...
    setRelativeDirInput(value.replace(/^\/+/, ''));
  };
              ...
              <div className="grid grid-cols-3 gap-2">
                {templates.length === 0 &&
                  Array.from({ length: 3 }).map((_, i) => (
                    <div
                      key={i}
                      className="p-3 rounded-lg border border-border flex flex-col items-center gap-2"
                    >
            ...
            <>
              ...
              nodeConfig?.workingDir ? (
                <div className="bg-muted/50 rounded-lg p-3 space-y-1">
                  ...
                  {nodeConfig?.env &&
                    Object.entries(nodeConfig.env).map(([key, value]) => {
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Input
                    value={relativeDir}
                    onChange={(e) => {
                      handleRelativeDirChange(e.target.value);
                    }}
                    className="h-9 text-sm font-mono"
                    placeholder={createName || 'my-loop'}
                  />
                ...
                {selectedTemplate.sessionSchema.envDefs.map((def) => (
                  <div key={def.key}>
                    <label className="text-xs text-muted-foreground flex items-center gap-1 mb-1">
                      ...
                      {def.placeholder && (
                        <span className="text-muted-foreground/60">— {def.placeholder}</span>
                      )}
                    </label>
                    <Input
                ...
                <div>
                  ...
                  <div className="mt-2">
                    <SessionPipeline steps={pipelineSteps} onChange={() => {}} readOnly />
                  </div>
                </div>
              </div>
