...
func TestCORS_OptionsRequest(t *testing.T) {
	...
	origin := rc.Response.Header.Get("Access-Control-Allow-Origin")
	if origin != "*" {
		t.Errorf("Access-Control-Allow-Origin = %q, 期望 %q", origin, "*")
	}
	methods := rc.Response.Header.Get("Access-Control-Allow-Methods")
	if methods == "" {
		t.Error("Access-Control-Allow-Methods 为空, 期望非空")
	}
	headers := rc.Response.Header.Get("Access-Control-Allow-Headers")
	if headers == "" {
