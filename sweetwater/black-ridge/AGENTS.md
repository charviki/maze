...
Go 后端入口在 cmd/black-ridge/，业务逻辑分 service/（Tmux 会话管理 + 心跳注册 + 配置读写）和 transport/（gRPC 适配 + WebSocket 终端）。路由采用 net/http + grpc-gateway + gRPC 统一架构，SPA 静态文件通过 go:embed 嵌入。
前端在 web/，嵌入后端镜像。
