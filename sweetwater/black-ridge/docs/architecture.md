...
┌──────────────────────────────────────────────────────────────┐
│                          main.go                              │
│  配置加载 → grpc-gateway ServeMux → gRPC Server → 路由注册    │
└──────────┬──────────┬──────────┬──────────┬──────────────────┘
           │          │          │          │
     ┌─────▼──┐ ┌─────▼──┐ ┌────▼─────┐ ┌▼──────────┐
     ...
     │  PTY / session / environment│ │ (NoRoute fwd) │
     └─────────────────────────────┘ └──────────────┘
```
...
7. **路由注册** — `register()` 注册 Hertz 中间件（CORS/AccessLog）、WebSocket 端点、SPA 静态文件、NoRoute 转发到 grpc-gateway
...
12. **信号处理** — 监听 SIGINT/SIGTERM，触发优雅关闭（关闭 stopCh → gRPC GracefulStop → 5s 超时 Hertz shutdown）
