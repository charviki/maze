...
1. **日志初始化** — 创建 slog 实例（`logutil.New("agent")`），替换 Hertz 默认 logger，统一 JSON 输出
2. **配置加载** — `config.LoadFromExe()` 搜索可执行文件所在目录及当前工作目录的 YAML 配置文件，执行解析 → 环境变量覆盖 → 校验
3. **HTTP 服务创建** — `server.Default(server.WithHostPorts(cfg.Server.ListenAddr))` 创建 Hertz 服务
4. **TmuxService 初始化** — 注入 TmuxConfig、状态目录、日志、TrustBootstrapper（Claude 信任注入）
5. **LocalConfigStore 初始化** — 加载/创建 `~/.maze/config.json`
6. **路由注册** — `register()` 注册业务路由（`biz/router/register.go`）和 SPA 静态文件（`go:embed web-dist`）
7. **心跳服务启动** — `HeartbeatService.Start()` 在独立 goroutine 中运行，向 Manager 注册并定期心跳
8. **自动保存服务启动** — `AutoSaveService.Start()` 在独立 goroutine 中定期保存所有活跃会话的管线状态
9. **信号处理** — 监听 SIGINT/SIGTERM，触发优雅关闭（关闭 stopCh → 5s 超时 shutdown）
10. **安全警告** — 若 `server.auth_token` 或 `controller.auth_token` 为空，打印 DEV 模式警告
...
3. **冲突检测** — 若当前 hash ≠ base_hash，返回 HTTP 409 + `ConfigConflictError`
