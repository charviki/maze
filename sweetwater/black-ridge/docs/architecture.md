...
- **控制面上报者** — 向 Director Core 注册并定期上报心跳，携带完整的 Agent 状态
...
└──────────┬──────────┬──────────┬──────────┬──────────────────┘
     ...
     └────┬───┘ └────┬───┘ └────┬─────┘ └──┬────────┘
          ...
          ▼          ▼          ▼            ▼
┌──────────────────┐ ┌──────────────────┐ ┌──────────────┐
│SessionState Repo │ │ConfigFile Provider│ │ grpc-gateway  │
│(状态持久化接口)   │ │(配置文件读写接口)  │ │ (ServeMux fwd)│
└────────┬─────────┘ └────────┬─────────┘ └──────────────┘
         │                    │
         ▼                    ▼
     ┌─────────────────────────────────┐
...
`service` 包作为业务核心层，遵循以下分层约定：

- **业务模型归属** — `Session`、`SessionState`、`SessionTemplate` 等业务模型定义在 `service` 包内，transport 层通过这些模型与 service 交互
- **SessionStateRepository** — 状态持久化通过 `SessionStateRepository` 接口隔离，默认实现为基于文件系统的 `session_state_fs.go`，transport 层不直接操作状态文件
- **ConfigFileProvider** — 配置文件读写通过 `ConfigFileProvider` 接口隔离，transport 层通过接口间接使用配置管理能力，具体实现可替换
- **transport 层约束** — `transport` 包仅负责 gRPC 协议适配和 WebSocket 终端路由，所有业务逻辑委托给 `service` 层的接口实现
...
8. **gRPC Server 启动** — 创建 gRPC Server，注册 3 个 Service（Session/Template/Config），应用 Proto 校验 → `UnaryAuthInterceptor` 认证拦截链
9. **Service 注册到 gateway** — 将 3 个 Service 注册到 grpc-gateway ServeMux（进程内直连，不走网络）
10. **心跳服务启动** — `HeartbeatService.Start()` 在独立 goroutine 中运行，向 Director Core 注册并定期心跳
...
| YAML 字段                     | 环境变量                            | 默认值 | 说明           |
| ----------------------------- | ----------------------------------- | ------ | -------------- |
| controller.addr               | AGENT_CONTROLLER_ADDR               | ""     | Director Core 地址 |
