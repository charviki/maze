...
| **Portal** | 乐园入口大门 | `mesa-hub/portal/` | ✅ 已实现 |
...
| **Fabrication** | 制造部，Host 的制造和维修场所 | `fabrication/skin/` | ✅ 已实现 |
...
Agent Manager（Go gRPC 后端 + React 前端）。通过 gRPC + grpc-gateway 管理 Agent 节点的注册、心跳、Host 声明式编排（HostSpec + Reconciler），代理前端到 Agent 的所有 HTTP 和 WebSocket 请求，记录审计日志。详见 [AGENTS.md](mesa-hub/behavior-panel/AGENTS.md)。

### Mesa-Hub / Portal — 入口门户

统一入口门户（Landing 含登录 → 主界面）。详见 [AGENTS.md](mesa-hub/portal/AGENTS.md)。

### Fabrication / Cradle — 共享基础库

Go 共享库，提供 gRPC/Protobuf IDL 代码生成、OpenAPI Go HTTP client 生成、grpc-gateway 工具（JSON 序列化、审计/鉴权拦截器）、统一日志、配置加载、管线模型、通信协议、脱敏工具和泛型 JSON 存储。详见 [AGENTS.md](fabrication/cradle/AGENTS.md)。

### Sweetwater / Black Ridge — Agent 运行时

Agent 节点服务（Go gRPC 后端 + 嵌入式前端）。通过 tmux 管理 AI CLI 工具的会话生命周期，提供 REST API + WebSocket 终端，支持三层 Pipeline 编排、会话状态持久化与恢复。详见 [AGENTS.md](sweetwater/black-ridge/AGENTS.md)。

### Fabrication / Skin — UI 组件库

Westworld 主题的 React 组件库。包含视觉特效组件、基础 UI 组件、Agent 业务组件，以及工具函数和 IAgentApiClient 接口。详见 [AGENTS.md](fabrication/skin/AGENTS.md)。
...
| 层面 | 技术选型 |
|------|----------|
| API 层 | Protobuf IDL + gRPC + grpc-gateway |
| Agent 后端 | Go 1.24 + tmux |
...
修改 `fabrication/skin` 后需重新 build（`cd fabrication/skin && pnpm run build`），消费方才会生效。修改 proto 文件后执行 `make gen` 重新生成 Go 类型和 HTTP client。
