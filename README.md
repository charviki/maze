...
The Mesa 汇总 Director Core（Go 控制核心）、Director Console（React 控制台前端）、Arrival Gate（统一入口前端）和 The Forge（知识库服务）。Director Core 通过 gRPC + grpc-gateway 管理 Agent 节点的注册、心跳，通过声明式 HostSpec + Reconciler 编排 Host 的完整生命周期，代理前端到 Agent 的所有 HTTP 和 WebSocket 请求，记录审计日志。详见 [AGENTS.md](the-mesa/AGENTS.md)。

### The Forge — 知识库服务

AI 与人共用的知识库服务，实现 Archive / Doc / DocLink 三层知识模型。一切皆文档——Doc 通过 parent_id 支持树形嵌套，通过 status 字段区分纯知识文档（null）与待办/任务（pending/active/done/failed）。支持全文搜索和 Markdown 编辑。详见 [AGENTS.md](the-mesa/the-forge/AGENTS.md)。
...
- pnpm
- buf（Proto 代码生成）
- openapi-generator + Java（HTTP client / TypeScript SDK 生成）
- Docker（镜像构建 + 运行时）
...
# 一键启动 K8s 开发环境
make up PLATFORM=kubernetes ENV=dev
make proxy PLATFORM=kubernetes ENV=dev

# 或 Docker Compose 环境
make up PLATFORM=docker ENV=dev|test|prod

# 单独启动 The Forge 前端开发服务器 (port 3001)
...
```bash
# Kubernetes（dev / test / prod 三套环境）
make up PLATFORM=kubernetes ENV=dev
make up PLATFORM=kubernetes ENV=test
make up PLATFORM=kubernetes ENV=prod
...
本项目使用 pnpm workspace 管理 Monorepo。`fabrication/skin` 是共享组件库，被 `the-mesa/director-console`、`the-mesa/arrival-gate`、`the-mesa/the-forge/web` 和 `black-ridge/web` 通过 `workspace:*` 引用。
