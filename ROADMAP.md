...
- [The Mesa](the-mesa/AGENTS.md) — 控制面域，汇总 Director Core、Director Console、Arrival Gate 与 The Forge
- [The Forge](the-mesa/the-forge/AGENTS.md) — 知识库后端服务（Archive / Doc / DocLink 三层知识模型）
- [Arrival Gate](the-mesa/arrival-gate/AGENTS.md) — 统一入口门户（Landing 含登录 → 主界面）
- [Director Console](the-mesa/director-console/AGENTS.md) — 控制台前端，负责节点、会话、模板等浏览器侧交互
...
### Abernathy Ranch — 语义检索

向量数据库与语义检索层（Dolores 的家庭农场，承载核心记忆）。The Forge 已实现结构化知识库（CRUD + pg_trgm 全文搜索），Abernathy Ranch 定位为向量嵌入与语义匹配的增强层。

**技术选型待定**：Qdrant / pgvector
...
```
Arrival Gate ──→ Director Console ──→ Director Core ──→ Black Ridge
                                           │                  │
                                      The Forge               │
                                           │                  │
                                           └──→ Cradle ←──────┘
...
- **Cradle** 是所有 Go 模块的共享基础，被 Director Core、The Forge 和 Black Ridge 共同依赖
- **Director Console** 通过 Director Core 代理前端请求到 **Black Ridge**
- **The Forge** 知识库服务通过 grpc-gateway 暴露 REST API，复用 Director Core 的 JWT 认证体系
- **Integration Tests** 跨模块验证 Director Core + Black Ridge + The Forge 的完整链路
